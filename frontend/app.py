    ...
    try:
        ...
        if os.path.exists(email_model_path):
            try:
                ...
                print(f"[OK] Email model (SVM + TF-IDF) loaded successfully from {email_model_path}")
            ...
            print(f"[WARNING] Email model not found at {email_model_path}, using heuristic analysis")
        ...
        if os.path.exists(url_model_path):
            try:
                ...
                print(f"[OK] URL model (RandomForest) loaded successfully from {url_model_path}")
            except Exception as e:
                print(f"[ERROR] Failed to load URL model from {url_model_path}: {e}")
                url_model = None
        else:
            print(f"[WARNING] URL model not found at {url_model_path}, using heuristic analysis")
            
        # Summary
        print("\n[INFO] Model Status Summary:")
        print(f"   Email Model: {'[OK] Loaded' if email_model is not None else '[WARNING] Using Heuristics'}")
        print(f"   URL Model: {'[OK] Loaded' if url_model is not None else '[WARNING] Using Heuristics'}")
        print(f"   FeatureExtraction: {'[OK] Available' if FeatureExtraction is not None else '[WARNING] Not Available'}")
